...
- `difficulty_target` は任意（`easy` / `medium` / `hard`）
  - 未指定時は難易度ターゲットによる選別を行わない
...
- 3x3/4x4 の妥当な問題を生成
- 一意解を保証
- （`difficulty_target` 指定時）目標難易度帯に近い問題を選別
- 仮定が必要な問題（上級）も生成可能
...
6. `difficulty_target` 指定時は目標条件を満たしたら採用、満たさなければ再試行
7. `difficulty_target` 未指定時は、難易度に関わらず品質基準を満たせば採用
